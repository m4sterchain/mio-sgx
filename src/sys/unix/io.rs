...
pub fn set_nonblock(fd: libc::c_int) -> io::Result<()> {
    unsafe {
        ...
        ::cvt_ocall(libc::ocall::fcntl_arg1(fd, libc::F_SETFL, flags | libc::O_NONBLOCK)).map(|_|())
    }
}
...
pub fn set_cloexec(fd: libc::c_int) -> io::Result<()> {
    unsafe {
        ...
        ::cvt_ocall(libc::ocall::fcntl_arg1(fd, libc::F_SETFD, flags | libc::FD_CLOEXEC)).map(|_| ())
    }
}
